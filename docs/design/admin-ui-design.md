...
> **2026-07-27 补齐了其中六项**（标 ✅ 的行）。挑这五项是因为它们是**事实字段**（时刻、计数、留痕、窗口参数），推得出来、不需要产品先定口径；剩下未补的两项不是懒得做：
> - ~~**健康指标条的「需关注」档**~~ —— 这一条我原先框错了：本页 `deriveStatus` **一直有一条规则在跑**，问题不是「没有定义」而是**定义放错了地方**（住在前端、只对当前页生效）。规则已原样搬到 server 并参数化，见上表。仍待产品的是「加不加别的维度、阈值多少」。
> - ~~**备用路由**~~ —— **已实现**（2026-07-27，migration `0051`）：`routes_json` 可声明单个 `fallback` profile，只在传输层失败时回退，回退次数落 `world_ticks.fallback_used`。⚠️ **「路由错误率」这一栏仍是 `—`**：数据源有了，但「错误率」的分母（每拍/每次调用/每世界）还要定一次。🔴 绝不能拿 tick 失败率顶替——回退成功的拍不算失败，用失败率看恰好把它整个漏掉。
>
> **前端已同步接上**（`admin/src/pages/WorldsMonitorConsole.tsx`）：五处 `TODO(接口缺字段)` 注释
> 相应删除或重写；`moderationLatency` 那条**保留为 null 但理由整条重写**（数据源已有，仍不下发的
> 三条理由见该处注释）。诊断的取数窗口**下推到服务端**（`?sinceMs=&limit=`），客户端那次过滤
> 降级为二次收窄（预览模式不经接口 + 改范围到请求回来之间的过渡态）。
> ⚠️ 措辞也跟着改了：Prompt 那一栏由「更新人 / 更新时间」改为**「激活人 / 激活时间」**——
> `prompt_versions` 上没有 `updated_by` 列，叫「更新人」会让人以为那张表上真有这么一列。
>
...
|---|---|---|
| 健康指标条 运行中/需关注/已熔断 | ✅ **已补**（2026-07-27） | `GET /admin/worlds/summary`（**全量**，不是当前页）。🔴 此前按「已加载世界」本地统计，翻页未完时三档**系统性偏小**，而偏小幅度取决于用户翻到第几页——那比没有这个数更糟，且**不是前端能修的**。分档规则由 server 逐条保留本页原有 `deriveStatus`（熔断 > 库状态 > 预算 > 运行中），阈值原样搬（`0.9` → `MUSE_ATTENTION_BUDGET_BP=9000`），**没有改判定**。⚠️ 一处故意差异：server 只认 `budget_day = 今天`，「昨天烧光、今天没跑」的世界不再算需关注（修 bug，口径同 `spentTokensTodayEffective`）。响应带 `attentionReasons[]`。🔴 汇总拿不到时**不回落到页内现算**——那个数会以「全量」的名义显示页内值，比空着更误导 |
...
| **处置申诉队列** | 状态筛选 + 表格 + 裁决弹窗（2026-07-27 补，见 §15.4）|
...
4. **禁用状态必须解释原因**。三个动作按钮的禁用条件全部来自接口字段，Tooltip 逐条说明
   （「仅可下架当前处于已过审的内容」「仅可对已过审内容发起再审」「已永久移除的内容不可恢复」）。
   一个不给理由的灰按钮会被读成后台坏了。
   ⚠️ 2026-07-27 起「位图资产没有文本再审通道」这条 Tooltip **已删除**：位图再审通道已接上
   （图片机审入队 + 人审工作台回写），四类主体的再审按钮判据统一为「当前是否已过审」。
   人审工作台对位图队列项附图（`subjectImageUrl`）——没有图的位图审核项等于让人盲审。
...
### 15.4 处置申诉队列（同 Tab 内下半区，2026-07-27 新增）

被下架的作者对**处置本身**提的异议。放在同一个 Tab 而不是新开页面：它和上半区是同一件事的
两侧（一侧下架、一侧申辩），同一个角色白名单，运营在同一个视口里就能从「我当初为什么下架」
翻到「作者说了什么」。

| 表达规则 | 理由 |
|---|---|
| 🔴 **不与「申诉复审」Tab 合并** | 那条受理「发布时被驳回」、改判即放行；这条受理「过审后被处置」、改判走恢复台阶写回下架前的状态并把台账翻成已恢复。界面合并会诱导运营用错的入口做错的改判，而错的那条**绕过恢复台阶** |
| 裁决弹窗内联「当初的下架理由（内部）」 | 人审要据以判断处置是否成立。它只出现在这里 |
| 答复框的占位文案写明「会原样展示给作者」 | 🔴 两种"理由"的分野在**填写的那一刻**就要说清楚：下架理由是内部备注（作者永不可见），申诉答复是对外答复（作者可见）。事后再解释就晚了 |
| `removed` 主体：改判项**禁用** + 警告卡 | 永久移除不可逆（位图连对象字节一并删除），服务端恒 409。界面提前说清楚，而不是让人点了才吃报错；同时明写救济路径是「作者重新发布」 |
| 已裁决行不给「裁决」按钮 | 重复裁决服务端 409；界面不摆一个注定失败的按钮 |
| 空队列 | 明写「该筛选条件下没有处置申诉（这是真实结果，不是加载失败）」 |
