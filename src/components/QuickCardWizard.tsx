...
//
// 【诚实划界 —— 2026-07-29 订正，原文是**过陈述**】
// §7 描述的「深层字段随剧情触发式解锁」（"你的角色刚才在秘境门口犹豫了——想调他的决策模型吗？"）
// 此前这里写的是「该触发源尚未接入」。逐条核过之后那句话是错的：
//
// ✅ **剧情事件回流早就接好了**：`engine-event` 的 `kind='narrative'` / `payload.kind='roundDone'`
//    携带完整 `SceneRecord`（含 `decisions[]` / `outcomes[]` / `events[]`），
//    且 `RoleDecision.characterId` == `CharacterCardV2.id`，「哪张卡刚才做了什么」不需要新造映射。
// ❌ 真正缺的是**另外两件**：
//    ① **判据**——「什么算值得提示的时刻」。唯一可靠的候选是底线拦截
//       （`outcomes[].result === 'invalid' && ruleRefs 含 'rule:bottom_line'`），它天然指向
//       深层的 decisionModel/valuePriorities，正是那句话的直译。
//       ⚠️ §7 那个**字面例子在本地轨造不出来**：桌面壳把 `locations` 硬编码为空表，
//       负责秘境准入的 `rule:move_admission` 永不触发；而「犹豫」在引擎里根本不是被建模的概念。
//       任何抄那句话做 UI 文案的实现都会是在声称一件没发生的事。
//    ② **提示规则**——什么时候提示（当场 / 回合结束 / 下次打开卡库）、提示什么、打不打断。
//       这三问决定它是一个注记还是一次干预，属产品决定，不是工程能替着定的。
//
// 所以本文件仍然只实现前端的渐进披露骨架：深层字段默认折叠 + 手动解锁。
// 🔴 落盘 provenance 里的 `triggerSourceWired: false` 保持不变——它记的是「触发**没有接进这个向导**」，
//    与上面「事件流本身通着」不矛盾；改它之前先把 ① ② 定下来。
