...
pub struct OutlineNode {
    ...
    /// **宿命时刻**（游戏时间轴，2026-07-29）：`Some(t)` = 这个节点在 `t` **到点就发生**，
    /// 不需要任何角色去推。来源是原著里这件事的先后顺序（提取管线的 `chapterOrder` 映射而来）。
    ///
    /// ══════════════════════════════════════════════════════════════════════
    /// 🔴 它划开了「遵循原著」与「自然推演」的界线
    /// ══════════════════════════════════════════════════════════════════════
    /// 在这个字段之前，主线**只能靠角色推**：`threshold` 累积 + `advance_when` 谓词命中才翻 Done。
    /// 于是「血洗黑角城」会因为在场的人都躲着走而**永远不发生**——那与「主线遵循原著」直接矛盾。
    ///
    /// 有了它：
    /// - 主线**会不会发生** → 原著说了算（到点必发）
    /// - 主线**怎么发生、谁在场、后果如何** → 角色的自然推演说了算
    ///
    /// 🔵 `None` = 老式节点，行为与本字段落地前**逐字节相同**（老模板、老存档零影响）。
    /// 两条路径可以共存：一个世界里既有到点必发的宿命节点，也有等人来推的软节点。
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub due_at: Option<i64>,
    /// 宿命节点**发生在哪个地点**。与 `due_at` 成对使用。
    ///
    /// 🔴 它是「碰撞」的另一半坐标：到点时**在这个地点的角色亲历它**，不在场的**只听说**。
    /// 缺省（`None` 或空）= 无地点维度 ⇒ 全员视作在场（退化为老世界的单场语义）。
    #[serde(default, skip_serializing_if = "Option::is_none")]
