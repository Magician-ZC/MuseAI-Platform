# 副本卡混装（Subplot Cards）— 实现规格【已被取代】

> ⚠️ **本规格已被 [`spec-world-ecosystem.md`](./spec-world-ecosystem.md)（世界生态总规格 v2）取代**（2026-07-25）。
> 核心假设变更：副本卡不再是「用户自由提取的 UGC 资产」，而是**官方世界结算产出的确定性道具**
> （官方专营 + 同星合成 + 仅自定义房消费）。
>
> **有效性地图（防误用，开发与 AI 协作者必读）**：
> - ❌ **业务假设全部作废**：§1 资产模型（UGC 发布）、§2 发布与审核、§6 星级与产出、§7 权属与分成、
>   §8 红线——以总规格 §10/§11 与 VALIDATION.md 为准，不得按本文实现。
> - ✅ **仅技术方案有效**：§3 容器世界（命名空间 `cardId:item_id`、地点缝合 seams/anchors/nexus）、
>   §4 装配与防刷（四段式种子、卡采样域）、§5 冲突与一致性——R2 自定义房装配的技术附录。
