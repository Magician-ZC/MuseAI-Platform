...
interface AgentSettingCardProps {
  ...
  /** 是否展示「按环节模型路由」下拉（§12.4，可选 modelId）。 */
  showModelId?: boolean;
}

// modelId 未设置时回退全局 selectedModelId；用空串代表「跟随全局默认」。
const FOLLOW_GLOBAL_MODEL = '';
...
const AgentSettingCard: React.FC<AgentSettingCardProps> = ({
  ...
  const initialFormValues = {
    ...
    ...(showModelId ? { modelId: agentConfig.modelId ?? FOLLOW_GLOBAL_MODEL } : {}),
  };

  const modelIdOptions = React.useMemo(
    () => [
      { value: FOLLOW_GLOBAL_MODEL, label: '跟随全局默认' },
      ...(store.models || []).map((m) => ({ value: m.id, label: m.name })),
    ],
    [store.models],
  );
  ...
  const handleSave = (values: any) => {
    if (showModelControls) {
      const nextConfig = {
        ...
        // 空串 → undefined，回退全局模型（不在 UI 宣称已分级，仅存偏好）。
        ...(showModelId ? { modelId: values.modelId ? values.modelId : undefined } : {}),
      };
  ...
  const handleReset = () => {
    const values = {
      ...
      ...(showModelId ? { modelId: defaultConfig.modelId ?? FOLLOW_GLOBAL_MODEL } : {}),
    };
    ...
    if (showModelControls) {
      const nextConfig = {
        ...
        ...(showModelId ? { modelId: defaultConfig.modelId } : {}),
      };
      store.setAgentConfig(agentId, nextConfig);
    }
    onResetPrompt();
            ...
            {showModelId && (
              <div style={{ marginTop: '16px' }}>
                <Form.Item
                  label="按环节模型路由 (Model)"
                  name="modelId"
                  tooltip="可选：为该环节单独指定模型。留空则跟随全局默认模型。模型被删除或失效时自动回退全局模型。"
                  style={{ marginBottom: 0 }}
                >
                  <Select style={{ width: '100%' }} options={modelIdOptions} />
                </Form.Item>
              </div>
            )}
          </div>
...
// 懒挂载分组：标题常驻，卡片（含重量级 Form）在展开前不挂载，避免设置页初次渲染负载过重。
const LazyConfigGroup: React.FC<{
  title: string;
  ...
  return (
    <>
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '24px' }}>
        {icon}
        <Title level={4} style={{ color: '#33312e', margin: 0, fontWeight: 600, fontFamily: '"Inter", "Roboto", -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif' }}>{title}</Title>
        <Button type="link" style={{ color: '#d97757', paddingInline: 4 }} onClick={() => setOpen((o) => !o)}>
          {open ? '收起' : `展开配置（${count} 项）`}
        </Button>
      </div>
      {open && children}
    </>
    ...
    <div style={{ display: 'flex', height: '100%', overflow: 'hidden', background: '#faf9f5' }}>
      ...
      <div id="settings-scroll-container" style={{ flex: 1, padding: '40px 48px', overflowY: 'auto', paddingBottom: 120 }}>
        <div style={{ maxWidth: 800, margin: '0 auto' }}>
          ...
          {/* 角色提取设置区域（P0.b 全书提取 + 角色测试） */}
          <section id="character-extraction-config" style={{ marginBottom: 48 }}>
            <LazyConfigGroup title="角色提取设置" count={6} icon={<TeamOutlined style={{ fontSize: '20px', color: '#d97757' }} />}>
            <AgentSettingCard
              title="逐章扫描器"
              agentId="characterScan"
              defaultPrompt={defaultCharacterScanPrompt}
              currentPrompt={store.characterScanPrompt}
              onSavePrompt={store.setCharacterScanPrompt}
              onResetPrompt={store.resetCharacterScanPrompt}
              helpText="全书提取管线第二阶段：逐章并发扫描，产出带原文逐字引用的角色 mentions（严格 JSON、温度 0）。"
              showModelId
            />

            <AgentSettingCard
              title="别名归并器"
              agentId="characterMerge"
              defaultPrompt={defaultCharacterMergePrompt}
              currentPrompt={store.characterMergePrompt}
              onSavePrompt={store.setCharacterMergePrompt}
              onResetPrompt={store.resetCharacterMergePrompt}
              helpText="第三阶段：判定别名是否指向同一角色，宁可漏并不可错并，结果全部进用户确认页。"
              showModelId
            />

            <AgentSettingCard
              title="重要度分层器"
              agentId="characterTiering"
              defaultPrompt={defaultCharacterTieringPrompt}
              currentPrompt={store.characterTieringPrompt}
              onSavePrompt={store.setCharacterTieringPrompt}
              onResetPrompt={store.resetCharacterTieringPrompt}
              helpText="第五阶段：复核边界角色的核心/重要/功能/过场分层。"
              showModelId
            />

            <AgentSettingCard
              title="DNA 合成师"
              agentId="characterSynthesis"
              defaultPrompt={defaultCharacterSynthesisPrompt}
              currentPrompt={store.characterSynthesisPrompt}
              onSavePrompt={store.setCharacterSynthesisPrompt}
              onResetPrompt={store.resetCharacterSynthesisPrompt}
              helpText="第六阶段：为每个角色合成十层 Character DNA V2，并在合成内完成矛盾审查。"
              showModelId
            />

            <AgentSettingCard
              title="互换测试评审员"
              agentId="characterSwapTest"
              defaultPrompt={defaultCharacterSwapTestPrompt}
              currentPrompt={store.characterSwapTestPrompt}
              onSavePrompt={store.setCharacterSwapTestPrompt}
              onResetPrompt={store.resetCharacterSwapTestPrompt}
              helpText="互换测试（北极星指标）：判断两角色在同一局势下的选择是否不可替换。"
              showModelId
            />

            <AgentSettingCard
              title="压力测试评审员"
              agentId="characterStressTest"
              defaultPrompt={defaultCharacterStressTestPrompt}
              currentPrompt={store.characterStressTestPrompt}
          ...
          {/* 知识包设置区域（P1 蒸馏） */}
          <section id="knowledge-config" style={{ marginBottom: 48 }}>
            <LazyConfigGroup title="知识包设置" count={3} icon={<ReadOutlined style={{ fontSize: '20px', color: '#d97757' }} />}>
            <AgentSettingCard
              title="思维包蒸馏师"
              agentId="knowledgeDistillMind"
              defaultPrompt={defaultKnowledgeDistillMindPrompt}
              currentPrompt={store.knowledgeDistillMindPrompt}
              onSavePrompt={store.setKnowledgeDistillMindPrompt}
              onResetPrompt={store.resetKnowledgeDistillMindPrompt}
              helpText="Mind Pack：从资料蒸馏出问题拆解、证据偏好、决策启发（decisionHeuristics）。"
              showModelId
            />

            <AgentSettingCard
              title="价值包蒸馏师"
              agentId="knowledgeDistillValue"
              defaultPrompt={defaultKnowledgeDistillValuePrompt}
              currentPrompt={store.knowledgeDistillValuePrompt}
              onSavePrompt={store.setKnowledgeDistillValuePrompt}
              onResetPrompt={store.resetKnowledgeDistillValuePrompt}
              helpText="Value Pack：蒸馏价值尺度与原则（principles）。"
              showModelId
            />

            <AgentSettingCard
              title="表达包蒸馏师"
              agentId="knowledgeDistillExpression"
              defaultPrompt={defaultKnowledgeDistillExpressionPrompt}
              currentPrompt={store.knowledgeDistillExpressionPrompt}
              onSavePrompt={store.setKnowledgeDistillExpressionPrompt}
              onResetPrompt={store.resetKnowledgeDistillExpressionPrompt}
              helpText="Expression Pack：蒸馏语言组织规则（expressionRules）。"
              showModelId
            />
            </LazyConfigGroup>
          </section>
          ...
          {/* 叙事引擎设置区域（P2 回合） */}
          <section id="narrative-config" style={{ marginBottom: 48 }}>
            <LazyConfigGroup title="叙事引擎设置" count={5} icon={<ExperimentOutlined style={{ fontSize: '20px', color: '#d97757' }} />}>
            <AgentSettingCard
              title="导演（设局）"
              agentId="narrativeDirector"
              defaultPrompt={defaultNarrativeDirectorPrompt}
              currentPrompt={store.narrativeDirectorPrompt}
              onSavePrompt={store.setNarrativeDirectorPrompt}
              onResetPrompt={store.resetNarrativeDirectorPrompt}
              helpText="回合第一步：布置局势，绝不替角色做决定。"
              showModelId
            />

            <AgentSettingCard
              title="角色决策器"
              agentId="narrativeDecide"
              defaultPrompt={defaultNarrativeDecidePrompt}
              currentPrompt={store.narrativeDecidePrompt}
              onSavePrompt={store.setNarrativeDecidePrompt}
              onResetPrompt={store.resetNarrativeDecidePrompt}
              helpText="活跃角色以第一人称输出结构化提案（意图/行动/是否发言/影响对象/愿付代价/预测），只提案不改状态。"
              showModelId
            />

            <AgentSettingCard
              title="行动仲裁器"
              agentId="narrativeArbiter"
              defaultPrompt={defaultNarrativeArbiterPrompt}
              currentPrompt={store.narrativeArbiterPrompt}
              onSavePrompt={store.setNarrativeArbiterPrompt}
              onResetPrompt={store.resetNarrativeArbiterPrompt}
              helpText="只裁决可行性/冲突/信息可见性/结果与硬约束，绝不重写角色意图。"
              showModelId
            />

            <AgentSettingCard
              title="场景写手"
              agentId="narrativeWriter"
              defaultPrompt={defaultNarrativeWriterPrompt}
              currentPrompt={store.narrativeWriterPrompt}
              onSavePrompt={store.setNarrativeWriterPrompt}
              onResetPrompt={store.resetNarrativeWriterPrompt}
              helpText="唯一的创作环节：把局势与仲裁结果写成沉浸式正文（默认温度 0.8）。"
              showModelId
            />

            <AgentSettingCard
              title="一致性审校器"
              agentId="narrativeCritic"
              defaultPrompt={defaultNarrativeCriticPrompt}
              currentPrompt={store.narrativeCriticPrompt}
              onSavePrompt={store.setNarrativeCriticPrompt}
              onResetPrompt={store.resetNarrativeCriticPrompt}
              helpText="复核人物一致性与因果质量，只给修订建议（revisionSuggestions），不直接改状态。"
              showModelId
            />
            </LazyConfigGroup>
          </section>
