...
- 迁移(`server/migrations/0001-0043`)启动时自动执行。
...
- 迁移文件 `server/migrations/0001-0043`,启动自动按版本号顺序执行(sqlx migrate)。
...
(cd server && cargo test)                                          # 862 passed(default,含黄金世界回归)
(cd server && cargo test --features billing,arena)                 # 940 passed
...
# ✅ 全量两个 feature 组合(与 SQLite 那遍**通过数相同**;校验于 2026-07-27)
MUSE_TEST_DATABASE_URL=postgres://$USER@localhost:5432/muse_test \
  cargo test --manifest-path server/Cargo.toml -- --test-threads=8            # 862 passed
MUSE_TEST_DATABASE_URL=postgres://$USER@localhost:5432/muse_test \
  cargo test --manifest-path server/Cargo.toml --features billing,arena -- --test-threads=8   # 940 passed
```
