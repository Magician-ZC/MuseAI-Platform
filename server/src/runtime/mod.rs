...
///
/// 📄 **待评审的那部分已决策化**（2026-07-28）：见 `docs/build/open-decisions.md` §9。
/// 逐条核过后它**不是「只差写」**，两条各自独立的理由：
/// ① 真正省钱的形态是**跨世界同环节合批**，那会让一个世界的下一拍取决于同批别的世界
///    ——改变玩家可观察的节奏，属产品决定而非性能调优；
/// ② 唯一的真实 `ModelClient` 是同步的 `HttpModelClient`，没有厂商凭据就**无法验证**
///    （同「黄金世界真实录制」那条）。写出来只能被测试替身跑，等于交付一份从未被真实验证过的
///    成本路径——本仓对 `ModerationProvider` 已有过同样处境，办法是如实标注而不是当成做完了。
/// 上面这段改造路径本身写得足够细，两条前提兑现后直接照它开工即可，不需要重新设计。
mod offpeak {
