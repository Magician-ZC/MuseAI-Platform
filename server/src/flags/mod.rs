...
/// 这个开关**曾经对任何人开过吗**（不是「现在开着吗」）。
///
/// ════════════════════════════════════════════════════════════════════════════
/// 🔴 它回答的是一个与 `is_enabled` 完全不同的问题
/// ════════════════════════════════════════════════════════════════════════════
///
/// `is_enabled` 问「**对这个人 / 这个世界**现在开着吗」。而有两类消费方要问的是
/// 「这块功能**有没有对任何人开放过**」：
///
/// - **指标三态**（`entry_not_open` / `no_data_in_window` / `ok`）。功能默认关闭时
///   直接报 `0%`，会得到「一个看起来棒极了（或糟透了）、实际上什么都没测的数」，
///   而 T1/T5 门槛恰恰要拿它决定继续/调整/停止。
/// - **运营面可见性**。按世界灰度开着时弹幕/举报**会真实落库**，而按全局解析会把
///   运营面判成 404 —— **弹幕进得来、撤不下去**；**举报进得来、处置进不去**，
///   审核闭环直接卡死。处置面的可见性必须跟「有没有人能提交」一致。
///
/// 判定 = 全局解析为开 **或** `runtime_flags` 里存在任何一条该开关 `enabled = 1` 的记录
/// （后者覆盖按世界 / 按用户灰度：大盘关着但 3 个世界开了，入口就是开过的）。
///
/// 🔴 **fail-safe 方向是 `false`**：查库失败 → 按「没开过」处理。于是指标报「没测过」
/// 而不是 `0%`——一个诚实的空缺，胜过一个精确的错数。
///
/// ════════════════════════════════════════════════════════════════════════════
/// 🔴 为什么收在这里
/// ════════════════════════════════════════════════════════════════════════════
///
/// 本判定此前在 **`annotations` / `ifline` / `livestage` / `social` 四个模块里各写了一遍**，
/// 其中一处的文档还明写着「口径逐字抄 `annotations::entry_ever_open`」——
/// **手抄被当成了实现方式**。四份都直接查 `runtime_flags`，等于在 `flags` 这个
/// 「开关的唯一读取入口」旁边开了四条旁路。
///
/// 它决定的是**指标诚不诚实**与**审核闭环开不开得了**，任一份漂移都不会有人立刻发现：
/// 指标会照常出数（只是那个数是假的），运营面会照常 404（只是本该可见）。
/// 源码级红线 `tests::red_line_only_flags_queries_the_runtime_flags_table` 扫死。
pub async fn entry_ever_open(db: &AnyPool, flag: &str) -> bool {
    if is_enabled(db, flag, FlagCtx::global()).await {
        return true;
    }
    sqlx::query("SELECT CAST(COUNT(*) AS BIGINT) AS n FROM runtime_flags WHERE flag = $1 AND enabled = 1")
        .bind(flag)
        .fetch_one(db)
        .await
        .ok()
