...
🔴 **提交纪律**：这个工作区**同时有别人未提交的改动**。
一律用 `git add <明确的文件列表>`，**禁止 `git add -A` / `git add .`**——
后者会把此刻别人手上的活一起提交，**没有任何提示**，只有事后审 diff 才看得见
（2026-07-28 实际发生过，三个提交卷进了别人约 1600 行 + 9MB 设计稿，见 VALIDATION §3.59）。
...
## 查缺陷之前先读一节（2026-07-28 补）

`docs/VALIDATION.md` **§3.8.1「可复用结论」**——那是 2026-07-27/28 一轮巡检的方法总结：
缺陷的三种形状（同一判定的 N 份拷贝 / 判定对当事人不可见 / **接缝**）、
「包含表还是排除表」这个决定失败方向的取舍、
以及**红线本身会骗人**的几种写法（`contains` 型断言、手工列举的扫描面、可被悄悄扩大的豁免）。

它下面的 §3.9–§3.32 是逐条事故记录，需要细节时再翻。**先读 §3.8.1，别从第一篇读起。**

📄 **§3.33 是覆盖图**：那一轮扫过哪些模块、没扫哪些、下一轮建议从哪开始（当前排头的是
引擎 `narrative` 9699 行、前端 `stores` 7098 行）。要接着查缺陷，从那张图挑，别从零决定。

🔵 其中一条值得写在这里：**结论为「无缺陷」也要记下来**——
既省下一个人重查，也说明「N 处手工判定」不必然是缺陷，得看那 N 处的差异有没有理由。
