...
//!
//! ════════════════════════════════════════════════════════════════════════════
//! ⚠️ **多实例下有重复做功窗口，如实记在这里**（`docs/VALIDATION.md` §3.56）。
//! `attempt` 的续号是 `MAX(attempt)+1` 且**没有唯一约束**；取件靠
//! `NOT EXISTS (… created_at >= 最近下限)` 挡重复，那是 TOCTOU——
//! 两个并发 sweep 都读到「最近没人动过」就会各领一份。
//!
//! - **今天不可达**：`spawn_sweeper` 是**一个进程一条循环**。
//! - **多实例滚动发布时可达**，后果是重复的模型调用（本层烧真 token）。
//! - **但不会失控**：撞号不抬高 `MAX(attempt)`，重试预算 `MUSE_SAFETY_L3_MAX_ATTEMPTS`
//!   仍然封顶，上界是「实例数 × 预算」而非无限。与 `ifline::sweep` 那条
//!   「无封顶补投 = 无限烧钱」不是一回事（那条已由 `0052` 的计数列封住）。
//!
