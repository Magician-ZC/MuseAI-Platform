...
///
/// ── 口径：**一个 world 实例 = 一个阶段**（2026-07-28 产品拍板，不再是过渡态）────────────────
/// 规格 §8 的原文是"每卡每阶段"。Saga 粒度此前挂在 `open-decisions.md` §3 上未决，
/// 现已定为 **A：一个世界实例就是一个阶段**，「连载」由**开新实例 + 阶段间继承**实现，
/// 而不是让一个实例横跨多个阶段。故本文件按 `(world_id, character_id)` **全量累计、不带时间窗**：
/// 一个世界实例从开局到终局即一个阶段，与规格原文等价。
///
/// 🔴 **这个决定不是为了省一列**。「阶段」这个词在系统里有 5 个以上的消费点——
/// 托梦配额（本文件）/ 结算 / 贡献分 / SLO 窗口 / 校准视图——改粒度等于**同时改这 5 处的含义**，
/// 而存量 `interventions` 行没有 `stage_no`，迁移那一刻「历史托梦算在哪个阶段」无解。
/// 收益（连载体验）用「阶段间继承」这种更局部的机制拿得到，不必动粒度。
/// 要推翻它，需要的证据是产品把连载定义成「同一个世界**不间断**地演下去」——
/// 那才必须让一个实例跨阶段。
///
...
///
/// ⚠️ 万一将来推翻上面那条决定：需把 `stage_no`（或 `saga_id`）落到 interventions 行上并加进
/// 计数 SQL 的 WHERE，口径即回到规格原文"每卡每阶段"；届时无需改动调用点、
/// 拒绝语义（`reject_reason = "quota"`）与响应结构。**留着这段是为了说清代价，不是待办**。
