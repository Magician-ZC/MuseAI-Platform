...
1b. **if 线没有对账式补偿**（新登记）：队列丢了任务只能靠陈旧线让玩家重点，没有像
   §15 第 3 层 `sweep` 那样按数据重算待办。做法是现成的（对账 `ifline_worlds.beat_count`
   与 `ifline_beats` 的最大 `beat_no`），但**它与陈旧线是两个层次**：陈旧线保证玩家不被锁死，
   对账保证「玩家没再点也能补上」。前者是安全底线，后者是体验优化，故先做前者。
