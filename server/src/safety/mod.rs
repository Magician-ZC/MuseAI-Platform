...
mod tests {
    ...
    fn red_line_narrative_projections_never_leave_the_db_unfiltered() {
        ...
        // ——那几条恰好都不在内联测试模块之后，那种截断刚好没削到它们。
        // 也就是说本棘轮挡的是「条数变了」，不是「扫描器一定还健康」；
        // 扫描器本身的正确性由 `testkit` 那一侧的注释与用例负责。
        //
        // 🔵 这个数**从 8 改成过 7**（2026-07-28），改的理由值得留着：当时的 8 里混进了
        // `runtime/golden.rs` 的一条回归快照查询——它是**整文件测试模块**
        // （`#[cfg(test)] mod golden;`），而 `production_sources` 那时只剥文件内的
        // `#[cfg(test)] mod X { .. }` 块、不认这种声明，于是把测试夹具当生产码扫了。
