  ...
  return (
      ...
      <div style={{ 
          ...
          <Space size={2}>
            <Tooltip title="从整本书扫描并分层生成角色卡（TXT / Markdown）">
              <Button
                type="text"
                size="small"
                icon={<TeamOutlined style={{ color: '#d97757' }} />}
                onClick={() => setIsExtractionWizardOpen(true)}
                className="background-extraction-button"
              >
                全书提取角色
              </Button>
            </Tooltip>
            <Tooltip title="管理知识包 / 思维包，并绑定到角色">
              <Button
                type="text"
                size="small"
                icon={<ReadOutlined style={{ color: '#d97757' }} />}
                onClick={() => setIsKnowledgeManagerOpen(true)}
                className="background-knowledge-button"
              >
                知识包
              </Button>
            </Tooltip>
        ...
        {isKnowledgeManagerOpen && (
          <KnowledgePackManager open onClose={() => setIsKnowledgeManagerOpen(false)} />
        )}
