...
| # | 欠什么 | 出处 | 关掉它需要什么 |
|---|---|---|---|
| A1 | 「绝不 blanket 标 `applied_tick`」没有端到端用例 | §3.41 | 造一个「跑完整一拍并在中途插入礼物」的夹具 |
| A2 | `write_atomic` 的 rename 失败清理路径没验到 | §3.35 | 需要跨文件系统或权限构造 |
| A3 | 卡录入时不该产出 1-2 字的 secret（另一道门） | §3.44 | 在卡发布/校准面加长度校验，形态同 §3.43 |
| A4 | `MarkdownEditorImpl` 的 `loadedPath` 是纯纵深防御，注入证明不了它今天在挡什么 | §3.38 | 需要能让重渲染慢于 800ms 的构造 |
| A5 | `admin/` **没有任何测试基建**（只有 tsc） | §3.37 | 引入 vitest（要加依赖，属决定不是实现） |
